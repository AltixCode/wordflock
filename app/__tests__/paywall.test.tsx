...
describe("Paywall", () => {
  ...
  it("renders a muted before card describing the free, ad-supported experience", async () => {
    const { getByText } = await renderWithProviders(<Paywall />);
    expect(getByText(t("freeCardTitle"))).toBeTruthy();
    expect(getByText(t("freeCardBody"))).toBeTruthy();
  });

  it("renders a distinct accent-toned after card listing the lifetime-unlock benefits", async () => {
    seed({ lifetime: LIFETIME });
    const { getByText } = await renderWithProviders(<Paywall />);
    expect(getByText(t("unlockCardTitle"))).toBeTruthy();
    expect(getByText(t("feat1Title"))).toBeTruthy();
  ...
  it("tells the user, in this app's own reworded copy, that nothing was found to restore", async () => {
    const restore = jest.fn().mockResolvedValue("none");
    seed({ restore });
    const { getByText } = await renderWithProviders(<Paywall />);
    await fireEvent.press(getByText(t("restorePurchases")));
    await waitFor(() => expect(getByText(t("noPriorPurchases"))).toBeTruthy());
  });
