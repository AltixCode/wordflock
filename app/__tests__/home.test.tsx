...
jest.mock("@/content/sync", () => ({
  syncContent: jest
    .fn()
    .mockResolvedValue({ feedReachable: true, packsAdded: [], problems: [] }),
...
describe("Home", () => {
  ...
  it("keeps submit unreachable until exactly four are chosen", async () => {
    ...
    for (const w of board().slice(0, 3))
  ...
  it("reveals every group and offers a share once the game is lost", async () => {
    ...
    for (let i = 0; i < MISTAKES_ALLOWED; i += 1) {
      for (const g of groups())
        await act(async () => fireEvent.press(getByLabelText(g.words[i]!)));
      await act(async () => fireEvent.press(getByText(t("submitGuess"))));
    }
  ...
  it('puts the "pick four" instruction above the tile grid, not below the submit button', async () => {
    // Regression test for TestFlight feedback: the hint used to sit under the
    // submit button, at the bottom of the screen, where a tester reported it
    // read as an afterthought. It must render before the first tile in
    // document order, which for this single-column layout is the same as
    // "visually above".
    const { toJSON, getByLabelText } = await renderBoard();
    const firstWord = board()[0];
    if (firstWord === undefined) throw new Error("board() returned no tiles");
    ...
    const visit = (node: unknown): void => {
      ...
      if (Array.isArray(n.children) && n.children.includes(t("pickFour"))) {
        order.push("pickFour");
      }
      if (
        typeof n.props?.accessibilityLabel === "string" &&
        n.props.accessibilityLabel === firstWord
      ) {
        order.push("firstTile");
      }
    ...
    // Sanity: the tile itself really is on screen (fails loudly instead of
    // the assertion below silently passing on an empty `order`).
    expect(getByLabelText(firstWord)).toBeTruthy();
    expect(order.indexOf("pickFour")).toBeGreaterThanOrEqual(0);
    expect(order.indexOf("firstTile")).toBeGreaterThanOrEqual(0);
    expect(order.indexOf("pickFour")).toBeLessThan(order.indexOf("firstTile"));
  });
});
